...
class backend {
    ...
    $systemprompt = get_config('local_xlate', 'openai_prompt') ?: "You are a helpful translation assistant. Return structured JSON following the provided function schema.";
    ...
    try {
            ...
            try {
                $resshort = (strlen($result) > 10000) ? substr($result, 0, 10000) . '...[truncated]' : $result;
                error_log('[local_xlate] RESPONSE httpcode=' . $httpcode . ' body: ' . $resshort);
            } catch (\Exception $ex) {
                // ignore
            }
            ...
            if ($httpcode < 200 || $httpcode >= 300) {
                return ['ok' => false, 'errors' => ['http_error' => $httpcode, 'body' => substr($result, 0, 200)]];
            }
