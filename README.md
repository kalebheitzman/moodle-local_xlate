...
## Manual Key Usage (Not Supported)

`local_xlate` does not support manual assignment of `data-xlate-key` or related attributes. Keys are always auto-generated by the JavaScript runtime based on DOM structure and context. Manual key assignment is not supported and will be ignored by the translator.

To exclude content from translation, use `data-xlate-ignore`.
